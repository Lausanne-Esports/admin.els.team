  ...
  <on-click-outside :do="close">
    <div class="flex flex-col">
      <div class="flex relative">
        <div ref="button" class="flex items-center text-darker-blue w-full bg-light-blue-grey h-12 rounded-l-lg px-4 mb-6 cursor-pointer" type="text" @click="open">
          <span v-if="value && items.length > 0">{{ getValue().name }}</span>
          <span class="text-darker-blue opacity-50" v-else>Select your options</span>
        </div>

        <div class="flex text-darker-blue items-center justify-center bg-light-blue-grey h-12 rounded-r-lg px-4 ml-px cursor-pointer" @click="open()">
        ...
        <div ref="dropdown" v-show="isOpen" class="shadow flex flex-col bg-white rounded-lg absolute m-0 p-2 pin-r pin-l z-30">
          <input
            class="block w-full text-darker-blue w-full bg-light-blue-grey h-12 rounded-lg px-4"
            type="text"
            v-model="search"
            ref="search"
            @keydown.esc="close"
